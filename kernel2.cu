...
int main()
{
    ...
    cudaMallocManaged(&memo, sizeof(int) * (1010000+1)); //コンスタントメモリは64KBまでなので無理そう
    ...
    cudaMemcpy(memo, host_memo, sizeof(int) * (1010000+1), cudaMemcpyHostToDevice);
    delete[] host_memo;
    ...
    //for (int jc = max_notes; 0 <= jc; jc--) {
        ...
        {
            for (int a = 0; a <= max_notes - jc - j; a++) //aのループもGPUでやれるようにしたい
            {
