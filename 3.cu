...
int main()
{
    ...
    dim3 grid(1, max_notes+1, m_num); //x: a, y: 未使用, z: m
    int current_max = 1, score;
    typeof_memo *memo, *host_memo = new typeof_memo[1010000 + 1], *points;
    cudaMallocManaged(&points, sizeof(typeof_memo) * (max_notes+1) * (m_num*threads_per_block));
    ...
    for (int jc = 0; jc <= max_notes; jc++) for (int j = 0; j <= max_notes-jc; j++)
    {
